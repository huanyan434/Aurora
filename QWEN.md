...
7. **前端约定**:
    ...
    13. 图标统一使用 `lucide-vue-next`，优先选择线性、统一的图标风格；仅在第三方组件已内置图标时保留其原有实现
    14. 新增或替换图标时优先复用现有 lucide 图标命名，保持全局视觉一致性
9. **Git 提交规范**:
    1. 每次完成一个可独立提交的改动后，先进行编译验证：前端修改后先执行前端构建；后端修改后先执行 `go build -o aurora-backend main.go`；若同时修改前后端，则两者都要通过后再考虑提交。
    2. 编译成功后，必须先询问用户是否提交 commit，再执行提交。
    3. commit message 格式必须为 `<type>: <subject>`，其中 `<type>` 只能使用以下类型：`feat`、`fix`、`to`、`docs`、`style`、`refactor`、`perf`、`test`、`chore`、`revert`、`merge`、`sync`。
    4. `fix` 表示直接自动修复并提交已修复的问题；`to` 表示只产出 diff、不自动修复，适合多次提交过程中的中间提交，最终修复提交时再用 `fix`。
    5. 提交描述应简洁、具体，直接说明本次改动目标，避免空泛表述。
    6. 使用 git 时要优先查看 `git status`、`git diff`，必要时结合 `git log` 了解近期提交风格后再提交。
