...
func wsHandler(c *gin.Context) {
	...
	for {
		...
		case "image_generate":
			handleWSImageGenerate(conn, userInfo, req, true) // 新生成图片需要保存用户消息
		case "image_edit":
...
func handleWSImageGenerate(conn *websocket.Conn, user utils.User, req WSRequest, saveUserMessage bool) {
	...
	if isGptImageModel {
		...
		validSizes := map[string][]string{
			...
			"VIP":  {"1024x1024", "1536x1024", "1024x1536", "2048x2048", "2048x1152", "1152x2048"},
			"SVIP": {"1024x1024", "1536x1024", "1024x1536", "2048x2048", "2048x1152", "1152x2048", "3840x2160", "2160x3840"},
		}
